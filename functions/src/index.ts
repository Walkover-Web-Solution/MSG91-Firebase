...
async function processWrite(change: functions.Change<functions.firestore.DocumentSnapshot>) {
    ...
    //     return transaction.update(msgRef, msg);
    // })
    return await msgRef.update(msg);


}
// functions.firestore.document('').onWrite
// export const processQueue = functions.handler.firestore.document.onWrite(
export const processQueue = functions.firestore.document(`${config.collection}/{messageId}`).onWrite(
    async (change) => {
        initialize();
        try {
